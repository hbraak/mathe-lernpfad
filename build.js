#!/usr/bin/env node
/**
 * Build-Script: YAML-Aufgaben → data.js
 * 
 * Liest alle freigegebenen Unit-Dateien aus units/approved/*.yaml
 * und generiert die data.js für die App.
 * 
 * Usage: node build.js
 */
...
function loadUnits() {
    ...
    for (const file of files) {
        ...
        if (!doc.unit?.id || !doc.unit?.title) {
            console.error(`❌ ${file}: Fehlt unit.id oder unit.title`);
            process.exit(1);
        }
        if (!doc.tasks || !doc.tasks.length) {
            console.error(`❌ ${file}: Keine tasks definiert`);
            process.exit(1);
        }
        
        let errors = 0;
        for (const t of doc.tasks) {
            if (!t.id) { console.error(`  ❌ ${file}: Task ohne id`); errors++; }
            if (!t.question) { console.error(`  ❌ ${file}: ${t.id} ohne question`); errors++; }
            if (t.type === 'choice') {
                if (!t.options) { console.error(`  ❌ ${file}: ${t.id} choice ohne options`); errors++; }
                if (t.correct === undefined) { console.error(`  ❌ ${file}: ${t.id} choice ohne correct`); errors++; }
            } else {
                if (!t.answer) { console.error(`  ❌ ${file}: ${t.id} freitext ohne answer`); errors++; }
            }
            if (!t.hints || t.hints.length < 1) {
                console.warn(`  ⚠️  ${file}: ${t.id} hat keine Hints`);
            }
        }
...
function generateDataJS(units) {
    let js = '// ============================================================\n';
    js += '// AUTO-GENERATED by build.js — DO NOT EDIT MANUALLY\n';
...
console.log(`📦 ${units.length} Units geladen:`);
...
console.log(`\n✅ data.js generiert (${(js.length/1024).toFixed(1)} KB)`);
...
try {
    ...
    console.error('❌ Syntax-Fehler in generierter data.js!');
